...
while count1 < ttotal:
	...
	hcells = np.loadtxt('Topografia%d.txt' %(count1))
	hcells0 = np.loadtxt('Topografia0.txt')
	
	hcells = hcells - hcells0
	...
	im1 = plt.imshow(grid_z0.T, origin='lower', extent=(xini*meterstokm,xfin*meterstokm,yini*meterstokm,yfin*meterstokm), vmin = -10.0, vmax = 10.0,cmap=cmap2)
	...
	plt.savefig('Dif_Topografia%d.png' %count1, dpi=500)
	plt.savefig('Dif_Topografia%d.pdf' %count1, dpi=500)
